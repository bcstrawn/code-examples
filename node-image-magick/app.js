...
var splitImage = function(path, callback) {
	var cmd = 'convert ' + path + ' -crop 16x16 -filter Point -resize x32 +antialias ' + path;
	//var cmd = 'convert cave/cave.png -crop 16x16 cave/cave.png';
	// convert cave.png -crop 16x16 -filter Point -resize x32 +antialias cave.png
...
var saveFile = function(req, callback) {
	...
	if (req.files.file.headers['content-type'] === 'image/png') {
		fs.rename(tempPath, targetPath, function(err) {
			...
			splitImage(relativePath, function() {
				callback();
			});
		});
	} else {
